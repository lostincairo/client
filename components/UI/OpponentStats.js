...
export default function OpponentStats() {
    ...
    const { data: health } = useStarknetCall({contract: game, method: "health_per_player_read", args: [opponent_address]});
    const HEALTH = health ? health.health_value.words[0] : [];

    const { data: action } = useStarknetCall({contract: game, method: "action_per_player_read", args: [opponent_address]});
    const ACTION = action ? action.action_value.words[0] : [];

    const { data: movement } = useStarknetCall({contract: game, method: "movement_per_player_read", args: [opponent_address]});
